...
public class CreateManager1 : MonoBehaviour {
    ...
    public UILabel mapNameInputtips;
	...
	void Start () {
        ...
        mapNameInputtips.text = "Please Input 1 - " + levelSum.ToString() + " to Rewrite the Map";
	}
    ...
    void Finsh(GameObject button)
    {
        ...
        //if (int.Parse(mapName.value.ToString()) > levelSum)
        //    mapName.value = levelSum.ToString();
        var now = System.DateTime.Now;
        if (mapName.value == "")
            saveMapName = now.Year+"-"+now.Month+"-"+now.Day+" "+now.Hour+now.Minute+now.Second+now.Millisecond;
        else
            saveMapName = mapName.value;
        saveMapName += ".map";

        new DataStoreProcessor().Save(mapData,CONSTANT.SaveDataPath+@"UserEdit/" + @saveMapName,true);
