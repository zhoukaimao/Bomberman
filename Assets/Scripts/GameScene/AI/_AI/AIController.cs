...
public class AIController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        dangerMap = _gameManager.GetDangerMap();
    ...
    public void ThinkForAWhile()
    {
        ...
        if (currentPath!=null&&currentPos + 1 < currentPath.Length)
        {
            ...
            if (GridIsDead(currentPath[currentPos + 1]))
            {
                isMoving = false;
            }
            if (!GridIsSafe(currentPath[currentPos + 1]) && currentPos>1)
            {
                isMoving = false;
            }
        }
    ...
    {
        if (player!=null&&Vector3.Magnitude(player.transform.position - transform.position) <= viewDist &&PathFindPlayer()!=null) return true;
        return false;
    }
    public Vector2[] PathFindDwall()
    {
        ...
        for (var r = 0; r <= radiu; r++)
        {
            for (var i = -r; i <= r; i++)
            {
                for (var j = -r; j <= r; j++)
                {
                    if (Util.ValidMapIndex(npcX + i, npcY + j) && CanWalkThrough(new Vector2(npcX + i, npcY + j)) && GridIsSafe(new Vector2(npcX + i, npcY + j)))
                    {
                        Vector2[] blastgrid = _gameManager.BlastGrid(npcX+i,npcY+j,flameDist);
                        foreach (Vector2 grid in blastgrid)
                        {
                            if (map[(int)grid.x, (int)grid.y] == CONSTANT.ObjEnum.Dwall)
                            {
                                path = PathFind(new Vector2(npcX + i, npcY + j));
                                if (path != null)
                                {
                                    return path;
                                }
...
<<<<<<< HEAD:Assets/Scripts/GameScene/AI/_AI/AIController.cs
=======
            if (maxGrade > 0) return PathFind(tempPos);
>>>>>>> parent of 3152d9f... Add map editor and UI:Assets/Scripts/GameScene/AI/AIController.cs
        }
        return null;
    }
        ...
        for (var r = 1; r <= radiu; r++)
        {
            for (var i = -r; i <= r; i++)
            {
                for (var j = -r; j <= r; j++)
                {
                    ...
                    if (Util.ValidMapIndex(npcX + i, npcY + j) && CanWalkThrough(new Vector2(npcX + i, npcY + j)) && GridIsSafe(new Vector2(npcX + i, npcY + j)))
                    {
                        Vector2[] path = PathFind(new Vector2(npcX+i,npcY+j));
    ...
    public bool GridIsSafe(Vector2 pos)
    {
        ...
        if (dangerMap[indexX, indexY] > 3000f) return true;
        else return false;
    }
    public bool GridIsDead(Vector2 pos)//very dangerous
    {
        if (dangerMap[(int)pos.x,(int)pos.y]<1000f) return true;
        else return false;
    }
